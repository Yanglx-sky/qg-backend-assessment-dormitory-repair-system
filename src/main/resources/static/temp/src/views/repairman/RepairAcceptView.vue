    ...
    <div class="order-list">
      ...
      <div v-else>
        <div v-for="order in orders" :key="order.orderId" class="order-item">
          ...
          <p>状态：<span class="order-status status-pending">{{ order.orderStatus }}</span></p>
          <p>提交时间：{{ order.createTime || new Date().toLocaleString() }}</p>
          <button class="btn" @click="acceptOrder(order.orderId)">接取订单</button>
...
const getPendingOrders = async () => {
  try {
    loading.value = true
    error.value = false
    const response = await axiosInstance.get('/repair-orders?status=待处理')
    if (response.success && response.data) {
      orders.value = response.data
    }
  } catch (err) {
    console.error('获取待接取订单失败:', err)
    error.value = true
  } finally {
    loading.value = false
  }
...
const acceptOrder = async (orderId) => {
  if (confirm('确定要接取这个订单吗？')) {
    try {
      ...
      const response = await axiosInstance.post(`/repair-orders/${orderId}/accept`, {
      ...
      if (response.success) {
        ...
        alert('订单接取失败：' + (response.message || '未知错误'))
      }
    } catch (err) {
      alert('订单接取失败，请稍后重试')
      console.error('接取订单失败:', err)
    }
  }
}
