    ...
    <div class="order-list">
      ...
      <div v-else>
        <div v-for="order in orders" :key="order.orderId" class="order-item">
          ...
          <!-- 为待处理的订单添加取消按钮 -->
          <button v-if="order.orderStatus === '待处理'" class="btn btn-cancel" @click="cancelOrder(order.orderId)">取消订单</button>
...
const getMyOrders = async () => {
  try {
    ...
    if (user) {
      const response = await axiosInstance.get(`/repair-orders?userId=${user.userId}`)
